...
for ($round = 1; $round -le $Rounds; ++$round) {
    ...
    foreach ($mode in $modesThisRound) {
        ...
        $paramsLines = Set-OrAddParamLine -Lines $paramsLines -Key "use_pinned_host_output" -Value $mode
        ...
        $tag = if ($mode -eq "true") { "output_pinned" } else { "output_pageable" }
        ...
        Write-Host ("[Round {0}/{1}] Running use_pinned_host_output={2}" -f $round, $Rounds, $mode)
        $sw = [System.Diagnostics.Stopwatch]::StartNew()
        & $ExePath $WeightsBin $paramsPath $outputBin
        $sw.Stop()
        if ($LASTEXITCODE -ne 0) {
            throw "Execution failed for use_pinned_host_output=$mode round=$round"
        }
        ...
        $hostOutputActive = Get-PerfValue -PerfLogPath $perfLog -Key "use_pinned_host_output"
        Write-Host ("  wall_process_ms={0:N6} kernel_time_ms={1:N6} end_to_end_ms={2:N6} graph_build_time_ms={3:N6} perf_param={4}" -f $wallMs, $kernelMs, $endToEndMs, $graphBuildMs, $hostOutputActive)

        Add-Result -Bucket $results -Mode $mode -Metric "wall_process_ms" -Value $wallMs
...
$summaryPath = Join-Path $OutputDir "pinned_output_ab.csv"

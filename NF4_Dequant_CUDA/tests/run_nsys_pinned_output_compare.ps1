...
function Write-ParamsWithPinnedOutputFlag {
    param(
        [string]$InputPath,
        [string]$OutputPath,
        [bool]$UsePinnedHostOutput,
    ...
    foreach ($line in $lines) {
        if ($line -match "^\s*use_pinned_host_output\s*=") {
            $out += "use_pinned_host_output = " + ($(if ($UsePinnedHostOutput) { "true" } else { "false" }))
            $foundPinnedOutput = $true
...
for ($r = 1; $r -le $Rounds; ++$r) {
    ...
    Write-ParamsWithPinnedOutputFlag -InputPath $ParamsTemplate -OutputPath $paramsPageable -UsePinnedHostOutput:$false -ProfileLoopIters $ProfileLoopIters
    Write-ParamsWithPinnedOutputFlag -InputPath $ParamsTemplate -OutputPath $paramsPinned -UsePinnedHostOutput:$true -ProfileLoopIters $ProfileLoopIters

    Write-Host "[2/4][$RunTag][Round $r/$Rounds] Nsight Systems profile (use_pinned_host_output=false)..."
    & $ProfileScript `
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "nsys profile failed for use_pinned_host_output=false, round=$r"
    }
    if (-not (Test-Path "$reportPageable.nsys-rep")) {
        throw "missing report: $reportPageable.nsys-rep"
    }

    Write-Host "[3/4][$RunTag][Round $r/$Rounds] Nsight Systems profile (use_pinned_host_output=true)..."
    & $ProfileScript `
        -WeightsBin $WeightsBin `
        ...
        -UseCudaProfilerRange $UseCudaProfilerRange
    if ($LASTEXITCODE -ne 0) {
        throw "nsys profile failed for use_pinned_host_output=true, round=$r"
    }
...
$summaryCsv = Join-Path $OutputDir ("nsys_{0}_pinned_output_median.csv" -f $RunTag)
...
Write-Host "Report stems (use_pinned_host_output=false):"
foreach ($s in $baseStems) {
    Write-Host "  $s"
}
Write-Host "Report stems (use_pinned_host_output=true):"
